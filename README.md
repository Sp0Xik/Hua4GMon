...
- **График** на голом `tk.Canvas` (без matplotlib — меньше .exe).
...
- **Полностью portable:** ничего не сохраняется на диск (нет config.ini,
  паролей, логов). Закрыли — и следов не осталось.

## Запуск

### Готовый .exe (Windows)

1. Скачать `Hua4GMon-vX.Y.Z-windows-x64.zip` из [Releases].
2. Распаковать в любую папку.
3. Запустить `Hua4GMon.exe`.
...
**Для параноиков:** в Release-постах публикуется SHA256-хеш zip-архива.
Можно проверить:

```powershell
Get-FileHash .\Hua4GMon-v1.2.0-windows-x64.zip
```
...
pyinstaller --onedir --windowed --noupx --name Hua4GMon main.py
```

Готовый exe в `dist/Hua4GMon/Hua4GMon.exe`. Используйте `--onedir`, а не
`--onefile`: реже триггерит SmartScreen и антивирусы.
...
В `tests/test_pure.py` ~76 тестов на чистые функции (парсинг бандов,
EARFCN, IP-валидация, оценка сигнала, анализ белых списков и т.д.).
Тесты не требуют сети или роутера.
...
Конфигурация в `pyproject.toml`. Те же команды гоняются в CI на каждый
push.

## Безопасность

Программа НЕ отправляет никаких данных «наружу», кроме:
- запросов на сам роутер по введённому IP (через `huawei-lte-api`);
- открытия страницы CellMapper в браузере по нажатию кнопки;
- TCP-проб на :443 на сайты из вкладки «Белые списки» — только когда
  пользователь нажмёт «Проверить».

Никакой телеметрии, никакой записи на диск, никаких сторонних SDK.
