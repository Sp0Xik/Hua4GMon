...
class Hua4GMon:
    def __init__(self, root):
        ...
        self.status_label.pack(anchor='center', pady=1)  # Минимальный отступ, как между Connect и прогрессбаром
        ...
        self.ax.set_title("Уровень сигнала", fontsize=10, pad=5)
        self.ax.set_xlabel("Время (сек)", fontsize=10, labelpad=2)
        self.ax.set_ylabel("Значение", fontsize=10, labelpad=2)
        self.ax.grid(True)
        ...
        if not default and self.start_time is not None:
            ...
            if param in data:
                ...
                try:
                    ...
                    self.ax.set_title(f"Уровень сигнала ({param.upper()})", fontsize=10, pad=5)
                    self.ax.set_xlabel("Время (сек)", fontsize=10, labelpad=2)
                    self.ax.set_ylabel(f"Значение ({self.get_unit(param)})", fontsize=10, labelpad=2)
                    self.ax.grid(True)
