...
class Hua4GMon:
    def __init__(self, root):
        ...
        self.ax.set_xlabel("Время (сек)", fontsize=10, labelpad=10)
        self.ax.set_ylabel("Значение", fontsize=10, labelpad=10)
        self.ax.grid(True)
        ...
        if not default and self.start_time is not None:
            ...
            if param in data:
                ...
                try:
                    ...
                    self.ax.set_xlabel("Время (сек)", fontsize=10, labelpad=10)
                    self.ax.set_ylabel(f"Значение ({self.get_unit(param)})", fontsize=10, labelpad=10)
                    self.ax.grid(True)
